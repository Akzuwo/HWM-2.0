...
function initialiseCalendar(events) {
  ...
  const calendar = new FullCalendar.Calendar(calendarEl, {
    ...
    moreLinkContent: (args) => `+${args.num} ${t('moreLink', 'more')}`,
    buttonText: {
...
async function loadCalendar() {
  const calendarEl = document.getElementById('calendar');
  if (!calendarEl) return;
  calendarEl.textContent = t('status.loading', 'Loading calendar …');
  ...
  try {
    ...
    calendarEl.textContent = t('status.error', 'Unable to load calendar entries!');
    hideApiHint();
  }
}
