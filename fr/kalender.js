...
function initialiseCalendar(events) {
  ...
  const calendar = new FullCalendar.Calendar(calendarEl, {
    ...
    moreLinkContent: (args) => `+${args.num} ${t('moreLink', 'autres')}`,
    buttonText: {
      month: t('views.month', 'Mois'),
      week: t('views.week', 'Semaine'),
      day: t('views.day', 'Jour')
    },
    events,
...
async function loadCalendar() {
  const calendarEl = document.getElementById('calendar');
  if (!calendarEl) return;
  calendarEl.textContent = t('status.loading', 'Chargement du calendrier …');
  ...
  try {
    ...
    calendarEl.textContent = t('status.error', 'Impossible de charger les entrées du calendrier !');
    hideApiHint();
  }
}
