    ...
    if (prevState.classSelectorEnabled !== nextState.classSelectorEnabled) {
      if (nextState.classSelectorEnabled) {
        initialiseClassSelector()
          .then(() => loadCalendar())
          .catch((error) => {
            console.error('Impossibile aggiornare la selezione della classe:', error);
          });
      } else {
        hideClassSelector();
        loadCalendar().catch((error) => {
          console.error('Impossibile aggiornare il calendario dopo il cambio di ruolo:', error);
        });
      }
    }
  });
} else {
  console.warn('Strumenti di permesso hmCalendar non disponibili. Ruolo ospite utilizzato.');
}
...
const classSelectorText = {
  ...
  placeholder: t('classSelector.placeholder', 'Seleziona una classe'),
  loading: t('classSelector.loading', 'Caricamento delle classi…'),
  error: t('classSelector.error', 'Impossibile caricare le classi.'),
  changeError: t('classSelector.changeError', 'Impossibile cambiare classe.'),
  required: t('classSelector.required', 'Seleziona una classe per visualizzare il calendario.')
};
...
const TYPE_LABELS = {
  ...
  ferien: t('legend.holiday', 'Vacanze e festività')
};
...
const modalText = {
  ...
  deleteSuccess: modalT('messages.deleteSuccess', 'Voce eliminata con successo.'),
  ...
  function setCurrentClassContext(classId, classSlug) {
    ...
    if (picker && typeof picker.syncWithCurrentClass === 'function') {
      try {
        ...
        console.warn('Impossibile sincronizzare il selettore di classi:', error);
      }
    }
  }
...
async function fetchSessionClassContext() {
  try {
    ...
    console.error('Impossibile caricare il contesto classe:', error);
    return null;
  }
}
...
async function fetchAvailableClasses() {
  try {
    ...
    console.error('Impossibile caricare l’elenco delle classi:', error);
    throw error;
  }
}
...
async function updateSessionClassSelection(slug, { silent = false } = {}) {
  ...
  try {
    ...
    console.error('Impossibile aggiornare la classe:', error);
    if (!silent) {
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  (async () => {
    try {
      ...
      console.error('Impossibile inizializzare il calendario:', error);
    }
  })();
});
