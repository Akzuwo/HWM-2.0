        ...
        <div class="login-container" role="dialog" aria-modal="true" aria-labelledby="auth-overlay-title">
            ...
            <header class="login-header">
                ...
                <div class="login-heading">
                    ...
                    <span class="login-badge" data-auth-register-only data-auth-display="inline-flex" aria-hidden="true" hidden>${LOGIN_TEXT.newBadge || 'NEU'}</span>
                </div>
...
async function verifyCode(form) {
    ...
    try {
        ...
        console.error('Code-Verifizierung fehlgeschlagen', error);
        setLoginFeedback(LOGIN_TEXT.verificationCodeInvalid, 'error', targetForm);
    } finally {
        setFormLoading(targetForm, false);
    }
...
async function resendVerification(form) {
    ...
    try {
        ...
        console.error('Code konnte nicht erneut gesendet werden', error);
        setLoginFeedback(LOGIN_TEXT.resendError, 'error', targetForm);
    } finally {
        if (button) {
...
async function submitPasswordResetConfirmation(form) {
    ...
    try {
        ...
        console.error('Passwort-Zurücksetzen bestätigen fehlgeschlagen', error);
        setLoginFeedback(LOGIN_TEXT.passwordResetError, 'error', targetForm);
    } finally {
        if (submitButton) {
...
const ENTRY_FORM_MESSAGES = {
    ...
    invalidEndDate: 'Das Enddatum darf nicht vor dem Startdatum liegen.',
    ...
    classLoadError: 'Die Klassen konnten nicht geladen werden.'
};
    ...
    return {
        async prepare() {
            ...
            try {
                ...
                console.error('Eintragsklassen konnten nicht geladen werden:', error);
...
async function showEntryForm(defaults = null) {
    ...
    if (window.hmEntryClassPicker && typeof window.hmEntryClassPicker.prepare === 'function') {
        try {
            ...
            console.error('Auswahl der Klassen konnte nicht vorbereitet werden:', error);
        }
    }
...
async function refreshCalendarEntries() {
    try {
        ...
        console.error('Kalender konnte nach dem Speichern nicht aktualisiert werden:', error);
    }
