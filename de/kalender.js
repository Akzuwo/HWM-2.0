...
const classSelectorText = {
  label: t('classSelector.label', 'Klasse'),
  placeholder: t('classSelector.placeholder', 'Klasse wählen'),
  loading: t('classSelector.loading', 'Klassen werden geladen …'),
  error: t('classSelector.error', 'Klassen konnten nicht geladen werden.'),
  changeError: t('classSelector.changeError', 'Klasse konnte nicht gewechselt werden.'),
  required: t('classSelector.required', 'Bitte wähle eine Klasse, um den Kalender zu laden.')
};
...
const actionText = {
  exportLabel: t('actions.export.label', 'Exportieren'),
  exportLoading: t('actions.export.loading', 'Exportieren …'),
...
const modalText = {
  ...
  subjectLabel: modalT('labels.subject', 'Fach'),
...
const modalButtons = {
  save: modalT('buttons.save', 'Speichern'),
  saveLoading: modalT('buttons.saveLoading', 'Speichern …'),
  delete: modalT('buttons.delete', 'Löschen'),
  deleteLoading: modalT('buttons.deleteLoading', 'Löschen …')
};
...
async function fetchSessionClassContext() {
  try {
    ...
    console.error('Klassenkontext konnte nicht geladen werden:', error);
    return null;
  }
}
...
async function fetchAvailableClasses() {
  try {
    ...
    console.error('Klassenliste konnte nicht geladen werden:', error);
    throw error;
  }
}
...
function updateWeekStrip(calendar) {
  ...
  button.innerHTML = `
    <span>${t('weekStrip.week', 'KW')} ${weekNumber}</span>
...
async function loadCalendar() {
  const calendarEl = document.getElementById('calendar');
  if (!calendarEl) return;
  showCalendarLoading(calendarEl, t('status.loading', 'Kalender wird geladen …'));
  ...
  try {
    ...
    showCalendarError(calendarEl, t('status.error', 'Fehler beim Laden der Kalendereinträge!'));
  }
}
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  (async () => {
    try {
      ...
      console.error('Initialisierung des Kalenders fehlgeschlagen:', error);
    }
  })();
});
